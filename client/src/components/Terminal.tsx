...
export default function Terminal({ projectId, onFilesChanged }: TerminalProps) {
  ...
  useEffect(() => {
    // Only focus if terminal is visible (not hidden by parent)
  ...
  return (
    <div className="flex flex-col h-full bg-ide-bg-primary overflow-hidden" data-testid="terminal">
      {/* Terminal Header */}
      <div className="flex items-center justify-between px-2 md:px-3 py-1 md:py-2 bg-ide-bg-secondary border-b border-ide-border">
        ...
        <div className="flex items-center space-x-1">
          <Button
            ...
            className={`p-1 h-6 w-6 ${hideWarnings ? 'text-ide-success' : 'text-ide-text-secondary'}`}
      ...
      {/* Terminal Content */}
      <ScrollArea className="flex-1 p-1 md:p-3 terminal-container" ref={scrollAreaRef}>
        <div className="font-mono text-xs md:text-base space-y-1 w-full max-w-full">
          {lines.map((line) => (
            <div
              key={line.id}
