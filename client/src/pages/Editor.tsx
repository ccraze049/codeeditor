...
export default function Editor() {
  ...
  // Query for files with real-time refresh for terminal sync
  const { data: files, isLoading: filesLoading } = useQuery<File[]>({
    queryKey: ["/api/projects", projectId, "files"],
    retry: false,
    enabled: !!projectId && !isSharedView,
    staleTime: 1000, // 1 second - allow frequent updates for terminal sync
    refetchOnWindowFocus: true, // Refresh when window gains focus
    refetchInterval: 2000, // Auto-refresh every 2 seconds when files might be changing
  });
        ...
        <div className={`flex-1 flex flex-col min-w-0 ${isMobile && !isSidebarCollapsed ? 'ml-0' : ''}`}>
          ...
          <div className="flex flex-1 overflow-hidden">
            <div className="flex-1 flex flex-col">
              ...
              {/* Bottom Panel */}
              {isBottomPanelOpen && (
                <div className={`${isMobile ? 'h-96' : 'h-80'} bg-ide-bg-secondary border-t border-ide-border flex flex-col flex-shrink-0`}>
