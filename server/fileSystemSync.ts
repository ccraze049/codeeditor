...
export class FileSystemSync {
  ...
  async syncProjectFiles(options: FileSystemSyncOptions): Promise<SyncResult> {
    ...
    try {
      ...
      const dbFiles = await storage.getProjectFiles(options.projectId);
      const dbFileMap = new Map(dbFiles.map(f => [f.path, f]));

      // Scan filesystem
      const fsFiles = await this.scanDirectory(
        options.projectPath,
        '/',
        0,
      ...
      for (const fsFile of fsFiles) {
        try {
          ...
          if (!dbFile) {
            ...
            await storage.createFile(fsFile);
            result.filesAdded++;
            console.log(`Added file: ${fsFile.path}`);
          } else {
      ...
      for (const entry of entries) {
        ...
        if (entry.name === 'node_modules') {
          if (includeNodeModules) {
            // Add node_modules as a folder but don't scan its contents deeply
            files.push({
              projectId,
              name: entry.name,
